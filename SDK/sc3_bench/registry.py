...
METHOD_REGISTRY = {
    ...
    #
    #   "external_kind" controls how the dispatcher in train.py invokes
    #   the method:
    #     - "base_method"  : the entry_point is a class implementing the
    #                        BaseMethod fit/predict protocol.
    #     - "custom"       : the entry_point is a torch model class with
    #                        a method-specific training driver. The SDK
    #                        does not auto-train these; see the method's
    #                        docstring or the README for the hand-written
    #                        runner.
    #     - "functional"   : the entry_point is a module exposing
    #                        train_*/predict_* functions (CLI wrappers).
    # ------------------------------------------------------------------
    "soltrannet":       {"display": "SolTranNet",        "family": "Transformer",  "featurizer": "smiles_tokens", "model_type": "external", "external_kind": "base_method", "entry_point": "sc3_bench.models.external.soltrannet:SolTranNetMethod"},
    "unimol":           {"display": "Uni-Mol2 + MLP",    "family": "Pretrained",   "featurizer": "unimol_repr",   "model_type": "external", "external_kind": "base_method", "entry_point": "sc3_bench.models.external.unimol_method:UniMolMethod"},
    "unimol_catboost":  {"display": "Uni-Mol2 + CatBoost","family": "Pretrained",  "featurizer": "unimol_repr",   "model_type": "external", "external_kind": "base_method", "entry_point": "sc3_bench.models.external.unimol_method:UniMolCatBoostMethod"},
    "unifac":           {"display": "UNIFAC + CatBoost", "family": "Group-Contrib","featurizer": "unifac_groups", "model_type": "external", "external_kind": "base_method", "entry_point": "sc3_bench.models.external.unifac_method:UNIFACMLModel"},
    "solvaformer":      {"display": "Solvaformer",       "family": "SE(3) Trans.", "featurizer": "smiles_3d",     "model_type": "external", "external_kind": "custom",      "entry_point": "sc3_bench.models.external.solvaformer:Solvaformer"},
    "rilood":           {"display": "RIL-OOD",           "family": "Invariant GNN","featurizer": "smiles_graph",  "model_type": "external", "external_kind": "custom",      "entry_point": "sc3_bench.models.external.rilood:RILOODModel"},
    "chemprop":         {"display": "Chemprop D-MPNN",   "family": "GNN",          "featurizer": "smiles_csv",    "model_type": "external", "external_kind": "functional",  "entry_point": "sc3_bench.models.external.chemprop_method"},
}
...
def get_hp(method_key: str) -> dict:
    ...
    External methods may not have an entry in best_hps.json; in that case
    we return an empty dict so the method can fall back to its own defaults.
    """
    all_hps = load_hps()
    if method_key not in all_hps:
        info = METHOD_REGISTRY.get(method_key, {})
        if info.get("model_type") == "external":
            return {}
...
def resolve_entry_point(entry_point: str):
    """Import and return a symbol referenced by ``entry_point``.

    Supports two forms used by the external method registry:
      * ``"package.module:Symbol"`` -> returns the named attribute.
      * ``"package.module"``        -> returns the module object.
    """
    import importlib

    if ":" in entry_point:
        mod_path, symbol = entry_point.split(":", 1)
