...
from .registry import METHOD_REGISTRY, EVAL_SPLITS, DEFAULT_SEEDS, get_hp, resolve_entry_point
...
# =====================================================================
# External baselines (sc3_bench.models.external)
#
# External methods are integrated via three protocols, distinguished by the
# ``external_kind`` field on the registry entry:
#
#   * "base_method"  : the registered class implements the BaseMethod
#                      interface (see sc3_bench/models/external/base.py).
#                      The dispatcher constructs ``cls(seed=seed, **params)``
#                      passing through any registry-resolved hyperparameters,
#                      then calls ``fit(splits["train"], splits["eval"])`` and
#                      ``predict(split)`` for each evaluation split. Each
#                      method is responsible for its own featurization given
#                      the train/eval/OOD DataFrames.
#   * "custom"       : the registered symbol is a torch model class with
#                      method-specific training requirements (e.g.
#                      Solvaformer, RIL-OOD). These ship the model and
#                      Dataset class only — invoke them from a hand-written
#                      training script rather than through this dispatcher.
#                      Calling ``train_method`` on a "custom" entry raises
#                      a NotImplementedError with a pointer to the
#                      method's source docstring.
#   * "functional"   : the registered symbol is a module exposing
#                      ``train_*`` / ``predict_*`` functions (chemprop's
#                      CLI wrapper). The dispatcher invokes the module's
#                      ``train_chemprop`` and ``predict_chemprop`` helpers.
# =====================================================================

def _train_external_seed(method_key, params, seed, splits):
    info = METHOD_REGISTRY[method_key]
    kind = info.get("external_kind", "base_method")
    target = resolve_entry_point(info["entry_point"])

    if kind == "custom":
        raise NotImplementedError(
            f"'{method_key}' is registered as model_type='external', "
            f"external_kind='custom'. The model class is exposed at "
            f"{info['entry_point']} but training is bespoke; see the "
            f"module docstring for the recommended driver."
        )

    if kind == "functional":
        # Chemprop-style module API. We expect the module to expose
        # train_chemprop(df_train, df_eval, work_dir, seed, **params) and
        # predict_chemprop(model_dir, df). The exact signatures are defined
        # in sc3_bench/models/external/chemprop_method.py.
        from pathlib import Path as _P
        work = _P(RESULTS_DIR) / method_key / f"_chemprop_workdir_seed_{seed}"
        work.mkdir(parents=True, exist_ok=True)
        model_dir = target.train_chemprop(splits["train"], splits["eval"], str(work),
                                          seed=seed, **(params or {}))
        meta = _split_meta(splits)
        metrics = {}
        for sname in EVAL_SPLITS:
            preds = target.predict_chemprop(model_dir, splits[sname])
            metrics[sname] = compute_metrics(
                meta[sname]["y_true"], preds,
                meta[sname]["solvent_names"], meta[sname]["uncertainties"],
            )
    ...
    # kind == "base_method"
    method = target(seed=seed, **(params or {}))
    df_tr, df_ev = splits["train"], splits["eval"]
    method.fit(df_tr, df_tr["LogS"].values, df_ev, df_ev["LogS"].values)

    meta = _split_meta(splits)
    metrics = {}
    for sname in EVAL_SPLITS:
        df = splits[sname]
        preds = method.predict(df)
        metrics[sname] = compute_metrics(
            meta[sname]["y_true"], preds,
            meta[sname]["solvent_names"], meta[sname]["uncertainties"],
        )
...
def train_method(method_key: str, seeds: list[int] | None = None,
    ...
    for seed in seeds:
        ...
        elif info["model_type"] == "external":
            model, metrics = _train_external_seed(method_key, params, seed, splits)
        else:
            raise ValueError(f"Unknown model_type '{info['model_type']}' for {method_key}")
